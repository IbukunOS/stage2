...
**Project Name:** Remote Hustle Live Infrastructure (Stage 2)

**URL:** <https://ec2-3-145-141-72.us-east-2.compute.amazonaws.com>
...
I implemented a CI/CD pipeline using GitHub Actions that automatically deploys code to the EC2 instance upon pushing to the `main` branch.
...
1. **Access:** Open `http://ec2-3-145-141-72.us-east-2.compute.amazonaws.com`. You will be redirected to HTTPS.
2. **Login:** Enter:

    `admin` / `admin123` (read-write)

    `staff` / `staff123` (read only)

 in the Session Authentication box and click "Set Session Role".
...
I implemented a **Staging vs Production** workflow:
